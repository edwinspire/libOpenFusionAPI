...
2.  **Application Variable Interpolation**: You can interpolate environment-specific Application Variables directly into the target URL string using double curly brackets:
    - *Example*: `https://api.service.com/{{ENV}}/orders`
3.  **Forwarding Rules**:
...
4.  **Response Handling**: The handler detects the content-type returned by the upstream service and forwards it directly back to the client (including binaries like PDFs or images).
